...
public final class VkDisplayPlaneInfo2KHR extends GroupType {
    ...
    /// @param segment the segment of the struct
    /// @param index the index of the struct buffer
    public static long mode(MemorySegment segment, long index) { return (long) VH_mode.get().get(segment, 0L, index); }
    /// {@return `mode`}
    ...
    /// @param index the index of the struct buffer
    /// @param value the value
    public static void mode(MemorySegment segment, long index, long value) { VH_mode.get().set(segment, 0L, index, value); }
