...
public final class VkDisplayPresentInfoKHR extends GroupType {
    ...
    public static final Supplier<VarHandle> VH_persistent = StableValue.supplier(() -> LAYOUT.arrayElementVarHandle(PathElement.groupElement("persistent")));
    ...
    /// @param segment the segment of the struct
    /// @param index the index of the struct buffer
    public static int persistent(MemorySegment segment, long index) { return (int) VH_persistent.get().get(segment, 0L, index); }
    /// {@return `persistent`}
    ...
    /// @param index the index of the struct buffer
    /// @param value the value
    public static void persistent(MemorySegment segment, long index, int value) { VH_persistent.get().set(segment, 0L, index, value); }
