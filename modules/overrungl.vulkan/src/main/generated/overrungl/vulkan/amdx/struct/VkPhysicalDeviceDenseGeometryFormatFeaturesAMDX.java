// This file is auto-generated. DO NOT EDIT!
package overrungl.vulkan.amdx.struct;
import java.lang.foreign.*;
import java.lang.foreign.MemoryLayout.PathElement;
import java.lang.invoke.*;
import java.util.function.*;
import org.jspecify.annotations.*;
import overrungl.struct.*;
import overrungl.util.*;

/// Represents `VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX`.
/// ## Layout
/// ```
/// struct VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX {
///     VkStructureType sType;
///     void* pNext;
///     VkBool32 denseGeometryFormat;
/// }
/// ```
public final class VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX extends GroupType {
    public static final StructLayout LAYOUT = LayoutBuilder.struct(
        ValueLayout.JAVA_INT.withName("sType"),
        ValueLayout.ADDRESS.withName("pNext"),
        ValueLayout.JAVA_INT.withName("denseGeometryFormat")
    );
    public static final long OFFSET_sType = LAYOUT.byteOffset(PathElement.groupElement("sType"));
    public static final long OFFSET_pNext = LAYOUT.byteOffset(PathElement.groupElement("pNext"));
    public static final long OFFSET_denseGeometryFormat = LAYOUT.byteOffset(PathElement.groupElement("denseGeometryFormat"));
    public static final MemoryLayout LAYOUT_sType = LAYOUT.select(PathElement.groupElement("sType"));
    public static final MemoryLayout LAYOUT_pNext = LAYOUT.select(PathElement.groupElement("pNext"));
    public static final MemoryLayout LAYOUT_denseGeometryFormat = LAYOUT.select(PathElement.groupElement("denseGeometryFormat"));
    public static final VarHandle VH_sType = LAYOUT.arrayElementVarHandle(PathElement.groupElement("sType"));
    public static final VarHandle VH_pNext = LAYOUT.arrayElementVarHandle(PathElement.groupElement("pNext"));
    public static final VarHandle VH_denseGeometryFormat = LAYOUT.arrayElementVarHandle(PathElement.groupElement("denseGeometryFormat"));

    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX(MemorySegment segment, long elementCount) { super(segment, LAYOUT, elementCount); }
    public static VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX of(MemorySegment segment) { return MemoryUtil.isNullPointer(segment) ? null : new VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX(segment, estimateCount(segment, LAYOUT)); }
    public static VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX ofNative(MemorySegment segment) { return MemoryUtil.isNullPointer(segment) ? null : new VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX(segment.reinterpret(LAYOUT.byteSize()), 1); }
    public static VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX ofNative(MemorySegment segment, long count) { return MemoryUtil.isNullPointer(segment) ? null : new VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX(segment.reinterpret(LAYOUT.scale(0, count)), count); }
    public static VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX alloc(SegmentAllocator allocator) { return new VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX(allocator.allocate(LAYOUT), 1); }
    public static VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX alloc(SegmentAllocator allocator, long count) { return new VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX(allocator.allocate(LAYOUT, count), count); }
    public static VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX allocInit(SegmentAllocator allocator) { return alloc(allocator).sType(overrungl.vulkan.amdx.VKAMDXDenseGeometryFormat.VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DENSE_GEOMETRY_FORMAT_FEATURES_AMDX); }
    public static VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX allocInit(SegmentAllocator allocator, long count) {
        var s = alloc(allocator, count);
        for (long i = 0; i < count; i++) s.sTypeAt(i, overrungl.vulkan.amdx.VKAMDXDenseGeometryFormat.VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DENSE_GEOMETRY_FORMAT_FEATURES_AMDX);
        return s;
    }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX copyFrom(VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX src) { this.segment().copyFrom(src.segment()); return this; }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX reinterpret(long count) { return new VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX(this.segment().reinterpret(LAYOUT.scale(0, count)), count); }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX asSlice(long index) { return new VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX(this.segment().asSlice(LAYOUT.scale(0L, index), LAYOUT), 1); }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX asSlice(long index, long count) { return new VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX(this.segment().asSlice(LAYOUT.scale(0L, index), LAYOUT.byteSize() * count), count); }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX at(long index, Consumer<VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX> func) { func.accept(asSlice(index)); return this; }
    public int sTypeAt(long index) { return (int) VH_sType.get(this.segment(), 0L, index); }
    public MemorySegment pNextAt(long index) { return (MemorySegment) VH_pNext.get(this.segment(), 0L, index); }
    public int denseGeometryFormatAt(long index) { return (int) VH_denseGeometryFormat.get(this.segment(), 0L, index); }
    public int sType() { return (int) VH_sType.get(this.segment(), 0L, 0L); }
    public MemorySegment pNext() { return (MemorySegment) VH_pNext.get(this.segment(), 0L, 0L); }
    public int denseGeometryFormat() { return (int) VH_denseGeometryFormat.get(this.segment(), 0L, 0L); }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX sTypeAt(long index, int value) { VH_sType.set(this.segment(), 0L, index, value); return this; }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX pNextAt(long index, MemorySegment value) { VH_pNext.set(this.segment(), 0L, index, value); return this; }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX denseGeometryFormatAt(long index, int value) { VH_denseGeometryFormat.set(this.segment(), 0L, index, value); return this; }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX sType(int value) { VH_sType.set(this.segment(), 0L, 0L, value); return this; }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX pNext(MemorySegment value) { VH_pNext.set(this.segment(), 0L, 0L, value); return this; }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX denseGeometryFormat(int value) { VH_denseGeometryFormat.set(this.segment(), 0L, 0L, value); return this; }
    public MemorySegment _sTypeAt(long index) { return this.segment().asSlice(LAYOUT.scale(OFFSET_sType, index), LAYOUT_sType); }
    public MemorySegment _sType() { return _sTypeAt(0L); }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX _sTypeAt(long index, MemorySegment src) { _sTypeAt(index).copyFrom(src); return this; }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX _sType(MemorySegment src) { return _sTypeAt(0L, src); }
    public MemorySegment _pNextAt(long index) { return this.segment().asSlice(LAYOUT.scale(OFFSET_pNext, index), LAYOUT_pNext); }
    public MemorySegment _pNext() { return _pNextAt(0L); }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX _pNextAt(long index, MemorySegment src) { _pNextAt(index).copyFrom(src); return this; }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX _pNext(MemorySegment src) { return _pNextAt(0L, src); }
    public MemorySegment _denseGeometryFormatAt(long index) { return this.segment().asSlice(LAYOUT.scale(OFFSET_denseGeometryFormat, index), LAYOUT_denseGeometryFormat); }
    public MemorySegment _denseGeometryFormat() { return _denseGeometryFormatAt(0L); }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX _denseGeometryFormatAt(long index, MemorySegment src) { _denseGeometryFormatAt(index).copyFrom(src); return this; }
    public VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX _denseGeometryFormat(MemorySegment src) { return _denseGeometryFormatAt(0L, src); }
}
