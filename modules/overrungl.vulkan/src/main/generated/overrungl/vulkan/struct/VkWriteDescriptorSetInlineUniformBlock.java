...
public final class VkWriteDescriptorSetInlineUniformBlock extends GroupType {
    ...
    /// @param segment the segment of the struct
    /// @param index the index of the struct buffer
    public static int dataSize(MemorySegment segment, long index) { return (int) VH_dataSize.get().get(segment, 0L, index); }
    /// {@return `dataSize`}
    ...
    /// @param index the index of the struct buffer
    /// @param value the value
    public static void dataSize(MemorySegment segment, long index, int value) { VH_dataSize.get().set(segment, 0L, index, value); }
