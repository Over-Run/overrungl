...
public final class VkQueryPoolCreateInfo extends GroupType {
    ...
    public static final Supplier<VarHandle> VH_queryType = StableValue.supplier(() -> LAYOUT.arrayElementVarHandle(PathElement.groupElement("queryType")));
    ...
    /// @param segment the segment of the struct
    /// @param index the index of the struct buffer
    public static int queryType(MemorySegment segment, long index) { return (int) VH_queryType.get().get(segment, 0L, index); }
    /// {@return `queryType`}
    ...
    /// @param index the index of the struct buffer
    /// @param value the value
    public static void queryType(MemorySegment segment, long index, int value) { VH_queryType.get().set(segment, 0L, index, value); }
