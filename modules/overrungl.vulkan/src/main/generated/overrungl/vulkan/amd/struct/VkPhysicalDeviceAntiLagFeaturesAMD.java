...
public final class VkPhysicalDeviceAntiLagFeaturesAMD extends GroupType {
    ...
    public static final Supplier<VarHandle> VH_antiLag = StableValue.supplier(() -> LAYOUT.arrayElementVarHandle(PathElement.groupElement("antiLag")));
    ...
    /// @param segment the segment of the struct
    /// @param index the index of the struct buffer
    public static int antiLag(MemorySegment segment, long index) { return (int) VH_antiLag.get().get(segment, 0L, index); }
    /// {@return `antiLag`}
    ...
    /// @param index the index of the struct buffer
    /// @param value the value
    public static void antiLag(MemorySegment segment, long index, int value) { VH_antiLag.get().set(segment, 0L, index, value); }
