...
public final class VkImportMemoryZirconHandleInfoFUCHSIA extends GroupType {
    ...
    /// @param segment the segment of the struct
    /// @param index the index of the struct buffer
    public static int handle(MemorySegment segment, long index) { return (int) VH_handle.get().get(segment, 0L, index); }
    /// {@return `handle`}
    ...
    /// @param index the index of the struct buffer
    /// @param value the value
    public static void handle(MemorySegment segment, long index, int value) { VH_handle.get().set(segment, 0L, index, value); }
