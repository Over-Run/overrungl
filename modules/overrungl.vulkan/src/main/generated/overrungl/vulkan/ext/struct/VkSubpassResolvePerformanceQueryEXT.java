...
public final class VkSubpassResolvePerformanceQueryEXT extends GroupType {
    ...
    public static final Supplier<VarHandle> VH_optimal = StableValue.supplier(() -> LAYOUT.arrayElementVarHandle(PathElement.groupElement("optimal")));
    ...
    /// @param segment the segment of the struct
    /// @param index the index of the struct buffer
    public static int optimal(MemorySegment segment, long index) { return (int) VH_optimal.get().get(segment, 0L, index); }
    /// {@return `optimal`}
    ...
    /// @param index the index of the struct buffer
    /// @param value the value
    public static void optimal(MemorySegment segment, long index, int value) { VH_optimal.get().set(segment, 0L, index, value); }
