...
public final class VkSamplerBorderColorComponentMappingCreateInfoEXT extends GroupType {
    ...
    public static final Supplier<VarHandle> VH_srgb = StableValue.supplier(() -> LAYOUT.arrayElementVarHandle(PathElement.groupElement("srgb")));
    ...
    /// @param segment the segment of the struct
    /// @param index the index of the struct buffer
    public static int srgb(MemorySegment segment, long index) { return (int) VH_srgb.get().get(segment, 0L, index); }
    /// {@return `srgb`}
    ...
    /// @param index the index of the struct buffer
    /// @param value the value
    public static void srgb(MemorySegment segment, long index, int value) { VH_srgb.get().set(segment, 0L, index, value); }
