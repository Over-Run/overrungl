...
public final class VkOutOfBandQueueTypeInfoNV extends GroupType {
    ...
    public static final Supplier<VarHandle> VH_queueType = StableValue.supplier(() -> LAYOUT.arrayElementVarHandle(PathElement.groupElement("queueType")));
    ...
    /// @param segment the segment of the struct
    /// @param index the index of the struct buffer
    public static int queueType(MemorySegment segment, long index) { return (int) VH_queueType.get().get(segment, 0L, index); }
    /// {@return `queueType`}
    ...
    /// @param index the index of the struct buffer
    /// @param value the value
    public static void queueType(MemorySegment segment, long index, int value) { VH_queueType.get().set(segment, 0L, index, value); }
